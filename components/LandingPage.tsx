...
import type * as ContentV2Module from "@/lib/content-v2";

export type LandingContent = Omit<typeof ContentModule, "hero" | "whySection"> & {
  hero: typeof ContentModule.hero | typeof ContentV2Module.hero;
  whySection: typeof ContentModule.whySection | typeof ContentV2Module.whySection;
};

type Props = {
  content: LandingContent;
  /** Prefixo nos IDs de CTA para tracking (ex.: "v2_"). */
  trackingPrefix?: string;
};

function isClassicHero(hero: LandingContent["hero"]): hero is typeof ContentModule.hero {
  return "titleCount" in hero;
}
...
function Avatars({ socialProofLabel }: { socialProofLabel: string }) {
  return (
    <div className="flex flex-col items-center gap-[5px]">
      ...
      <p className="text-[14px] font-semibold text-ink">{socialProofLabel}</p>
    </div>
  );
}

/* Carrossel dos materiais entregues (fichas de receita). */
function MaterialsCarousel({ materials }: { materials: typeof ContentModule.materials }) {
  return (
    <Marquee
      duration={32}
      itemWidth={260}
...
function TestimonialGrid({ testimonials }: { testimonials: typeof ContentModule.testimonials }) {
  return <Marquee duration={28} itemWidth={250} imageSize={testimonials.imageSize} items={testimonials.items} />;
}

/* ---------- página ---------- */

export function LandingPage({ content, trackingPrefix = "" }: Props) {
  const {
    theme,
    offerBar,
    hero,
  ...
  return (
    <main className="flex w-full flex-col" style={{ "--color-brand": theme.brandColor } as React.CSSProperties}>
      ...
      <StickyCta trackingPrefix={trackingPrefix} />

      {/* Hero */}
      <section className="mx-auto flex w-full max-w-[480px] flex-col items-center gap-[16px] px-[10px] pt-[10px] text-center">
        {"headline" in hero && hero.headline ? (
          <>
            {"tape" in hero && hero.tape ? (
              <Image
                src={hero.tape.src}
                alt={hero.tape.alt}
                width={hero.tape.width}
                height={hero.tape.height}
                sizes="(max-width: 480px) 95vw, 380px"
                className="w-full max-w-[380px] object-contain"
              />
            ) : null}
            <h1 className="max-w-[380px] font-heading text-[28px] font-bold leading-[1.15] text-ink sm:text-[32px]">
              {hero.headline}
            </h1>
          </>
        ) : isClassicHero(hero) ? (
        ...
        <CtaButton href="#plano-completo" label={hero.ctaLabel} id={`${trackingPrefix}hero`} className="max-w-[382px]" />
        ...
        <Avatars socialProofLabel={hero.socialProofLabel} />
      </section>
      ...
      <section className="w-full bg-brand px-[39px] py-[37px]">
        ...
        <div className="mx-auto flex w-full max-w-[1000px] flex-col gap-[11px]">
          {whySection.cards.map((c) => (
            <div
              key={"text" in c ? c.text : `${c.title}-${c.desc}`}
              className="flex w-full items-center gap-[14px] rounded-[10px] bg-white p-[16px]"
            >
              ...
              {"text" in c ? (
                ...
                <div className="flex flex-col gap-[4px] text-left">
                  <p className="font-heading text-[20px] font-bold leading-[1.21] text-ink">{c.title}</p>
                  <p className="font-display text-[16px] leading-[1.3] text-ink/85">{c.desc}</p>
                </div>
              )}
            </div>
          ))}
      ...
      <section className="mx-auto flex w-full max-w-[480px] flex-col items-center px-[10px] pb-[35px] pt-[35px]">
        ...
        <div className="mt-[-17px] flex w-full flex-col items-center gap-[14px] rounded-[16px] bg-dark px-[10px] py-[30px] text-center">
          ...
          />
          <FeatureList
            items={trackingPrefix === "v2_" ? offerSection.features : [...offerSection.features, ...bonusFeatures]}
            light
          />
          ...
          <CtaButton
            ...
            id={`${trackingPrefix}oferta_principal`}
            className={trackingPrefix === "v2_" ? "h-[75px]" : ""}
          />
      ...
      <section className="flex w-full flex-col items-center gap-[10px] bg-brand px-[39px] py-[37px]">
        <h2 className="mb-[10px] max-w-[640px] text-center font-heading text-[32px] font-bold leading-[1.109] text-white">
          {trackingPrefix === "v2_" ? (
            <>
              <span className="block">{bonusSection.titleLines[0]}</span>
              <span className="mt-[14px] block">{bonusSection.titleLines[1]}</span>
            </>
          ) : (
            <>
        ...
        <div className="grid grid-cols-1 justify-items-center gap-[16px] lg:grid-cols-3">
          {bonusSection.items.map((b) => (
            <article
              key={b.title}
              className={`flex w-full flex-col items-center gap-[13px] overflow-hidden rounded-[20px] bg-white pb-[20px] text-center ${trackingPrefix === "v2_" ? "max-w-[389px]" : "max-w-[324px]"}`}
            >
              <Image
                src={b.img}
                alt={b.title}
                width={b.width}
                height={b.height}
                sizes={trackingPrefix === "v2_" ? "389px" : "324px"}
                className={`h-[300px] w-full object-contain px-[12px] pt-[12px] ${trackingPrefix === "v2_" ? "bg-white" : "bg-surface"}`}
              />
              <h3
                className={`font-heading text-[22px] font-bold leading-[1.109] text-ink ${trackingPrefix === "v2_" ? "w-full max-w-none px-[16px]" : "max-w-[280px]"}`}
              >
                {b.title}
              </h3>
              <p className={`text-[16px] text-muted ${trackingPrefix === "v2_" ? "w-full max-w-none px-[16px]" : "max-w-[280px]"}`}>{b.desc}</p>
              {trackingPrefix === "v2_" ? (
                <div className="flex flex-col items-center gap-[8px] py-[4px]">
                  <span className="font-display text-[14px] font-semibold uppercase tracking-[0.06em] text-muted">Valor</span>
                  <s className="font-display text-[29px] font-semibold leading-none text-alert line-through">{b.price}</s>
                  <span className="inline-block rounded-full bg-badge px-[16px] py-[8px] text-[14px] font-bold text-white">
                    GRÁTIS
                  </span>
                </div>
              ) : (
              ...
              <p className={`font-display text-[15px] font-semibold leading-[1.008] text-alert ${trackingPrefix === "v2_" ? "w-full px-[16px]" : ""}`}>
      ...
      <section className="mx-auto flex w-full max-w-[480px] flex-col items-center px-[10px] pb-[35px] pt-[35px] lg:max-w-[1040px]">
        ...
        <div className="flex w-full flex-col items-center gap-[14px] rounded-[16px] border border-[#d5d5d5] bg-[#FCE9D8] px-[10px] py-[30px] text-center">
          ...
          <CtaButton href={CHECKOUT_BASIC} label={plansSection.basic.ctaLabel} id={`${trackingPrefix}plano_basico`} planName="Basico" />
          <Image
        ...
        <div id="plano-completo" className="mt-[40px] flex w-full scroll-mt-[20px] flex-col items-center lg:mt-0">
          ...
          <div className="mt-[-17px] flex w-full flex-col items-center gap-[14px] rounded-[16px] bg-dark px-[10px] py-[30px] text-center">
            ...
            <CtaButton href={CHECKOUT_COMPLETE} label={plansSection.complete.ctaLabel} id={`${trackingPrefix}plano_completo`} planName="Completo" />
            <Image
              src={plansSection.securePurchase.src}
              alt={plansSection.securePurchase.alt}
              width={plansSection.securePurchase.width}
      ...
      <section className="flex w-full flex-col items-center gap-[10px] bg-brand px-[39px] py-[37px] text-center">
        ...
        <CtaButton href="#plano-completo" label={stepsSection.ctaLabel} id={`${trackingPrefix}passo_a_passo`} className="mt-[10px]" />
      </section>
